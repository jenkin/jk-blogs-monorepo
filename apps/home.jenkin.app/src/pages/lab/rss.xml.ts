...
const imagesGlob = import.meta.glob<{ default: ImageMetadata }>(
  '/src/content/services/**/*.{jpeg,jpg,png,gif,avif,webp}' // add more image formats if needed
)
    ...
    return async function (tree: Root) {
      ...
      visit(tree, 'image', (node) => {
        if (node.url.startsWith('/images')) {
          ...
          const imagePathPrefix = `/src/content/services/${post.id}/${node.url.replace('./', '')}`
